...
type IndexData = Record<string, Map<string | number, string[] | number[]>>;

export default class TableUtilities {

  printField(fieldName?: string) {
    if (!fieldName) return `'${this.tableName}[${this.primaryKey}]'`;
    return `field '${this.tableName}[${this.primaryKey}].${fieldName}'`;
  }

  errorWrongId(id: string | number) {
  ...
  public scheme: SchemeRecord;

  static fromScheme(scheme: SchemeRecord) {
    //todo return from cache
    return new TableUtilities(scheme);
  }
  ...
  public relationIndex_Type: Record<number, FieldType>;
  public relationIndex_FieldName: string[];
  public relationFieldName_Index: Record<string, number>;

  constructor(scheme: SchemeRecord) {
    this.scheme = scheme;
    this.tableName = scheme.tableName;
    this.primaryKey = TableUtilities.getPrimaryKeyFromScheme(scheme);

    let numberOfType: Record<string, number> = {};

    this.relationIndex_Type = {};
    this.relationIndex_FieldName = [];
    this.relationFieldName_Index = {};

    for (let i = 0; i < scheme.fieldsOrder.length; i++) {
      let key = scheme.fieldsOrder[i];
      const type = this.scheme.fields[key];
      this.relationIndex_Type[i] = type;
      this.relationIndex_FieldName[i] = key;
      this.relationFieldName_Index[key] = i++;
  ...
  renameHeavyFiles<T extends string | number>(oldIds: T[], newIds: T[]) {
  ...
  buildIndexDataForRecords(records: any[]): IndexData
  buildIndexDataForRecords(records: any[], oldIds: string[] | number[]): [IndexData, IndexData]
  buildIndexDataForRecords(records: any[], oldIds?: any[]) {
    const result: IndexData = {}
    const resultOld: IndexData = {}
    ...
    this.forEachIndex((fieldName, dict, tags) => {
      ...
      const mapOld = new Map<string | number, any[]>();
  ...
  insertIndexData(data: IndexData) {
    for (const fieldName in data) {
      ...
      for (const [id, arr] of indexData) {
        ...
        if (!current) {
          ...
          const toInsert: any[] = arr.slice(0);
  ...
  fieldHasAnyTag(fieldName: string, ...tags: FieldTag[]) {
    return TableUtilities.tagsHasFieldNameWithAnyTag(this.scheme.tags, fieldName, ...tags);
  }

  fieldHasAllTags(fieldName: string, ...tags: FieldTag[]) {
    return TableUtilities.tagsHasFieldNameWithAllTags(this.scheme.tags, fieldName, ...tags);
  }

  getIndexDictDir(fieldName: string) {
    return `/${this.tableName}/indices/${fieldName}/`;
  }

  getMainDictDir() {
    return `/${this.tableName}/main/`;
  }

  getHeavyFieldDir(fieldName: string) {
    return `/${this.tableName}/heavy/${fieldName}/`;
  }
 ...
 */
  ...
  getPagesFilePath() {
    return `/${this.tableName}/pages.bin`;
  }

  getHeapFilePath() {
    return `/${this.tableName}/heap.bin`;
  }
}
